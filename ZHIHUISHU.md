...
- 视频支持完整视频及引用起止时间的片段；零时长的不可用视频和暂不支持的片段格式会提示手动处理，不上报完成。
- 已读取内容的电子书发送平台预览回执。引用 PPT 按文件后缀和引用类型识别，检查预览已就绪且引用页码存在，再发送预览回执并回查完成状态；其他资源类型提示手动处理。
- 每个知识点处理后重新查询全部资源并显示已完成数量；全部资源确认完成后自动进入下一知识点，章节内全部知识点完成后自动进入下一章节，无需手动切换。
- 发现未完成、不支持、消失或无法核验的任务时停止自动切换并提示处理。重新运行会跳过服务端已确认完成的资源，从未完成项继续；空目录不会被当作已完成。
